                                            ...
                                            NV_ENC_TUNING_INFO tuningInfo)
{
    ...
    NVENC_API_CALL(m_nvenc.nvEncGetEncodePresetConfigEx(m_hEncoder, codecGuid, presetGuid, tuningInfo, &presetConfig));
    ...
    if (tuningInfo != NV_ENC_TUNING_INFO_LOSSLESS)
    {
    ...
    {
        bool is10bit = (m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV420_10BIT || m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV444_10BIT);
        pIntializeParams->encodeConfig->encodeCodecConfig.hevcConfig.inputBitDepth = is10bit ? NV_ENC_BIT_DEPTH_10 : NV_ENC_BIT_DEPTH_8;
        pIntializeParams->encodeConfig->encodeCodecConfig.hevcConfig.outputBitDepth = is10bit ? NV_ENC_BIT_DEPTH_10 : NV_ENC_BIT_DEPTH_8;
        if (m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV444 || m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV444_10BIT)
        {
...
void NvEncoder::CreateEncoder(const NV_ENC_INITIALIZE_PARAMS* pEncoderParams)
{
    ...
    presetConfig.version = NV_ENC_PRESET_CONFIG_VER;
    presetConfig.presetCfg.version = NV_ENC_CONFIG_VER;
        m_nvenc.nvEncGetEncodePresetConfigEx(m_hEncoder, pEncoderParams->encodeGUID, NV_ENC_PRESET_P4_GUID,
