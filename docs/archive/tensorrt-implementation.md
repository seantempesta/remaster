...
- 362 val frames (mixed_val/input/)
- 3,382 train frames

**Recommendation:** Use all 362 val frames + a random subset of ~140-640 training frames = **500-1000 total**. The frames should cover diverse scene content (dark scenes, bright scenes, fast motion, static shots). For video denoising, temporal diversity matters more than quantity — 200 frames from different scenes is better than 500 frames from one scene.
...
1. Extract calibration frames: 200 frames from mixed_val + training data
...
5. Compare PSNR on mixed_val: fp16 PyTorch vs INT8 TRT (target < 0.3 dB drop)
...
- Calibrate: `IInt8EntropyCalibrator2` with 200-500 frames from mixed_val + training data
