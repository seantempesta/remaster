"""
Modal training wrapper for PlainDenoise / UNetDenoise.

Uploads training pairs to Modal volume, runs training on H100,
downloads the best checkpoint.

Usage:
    # Default: UNet nc=64 mid=2 (2.5M params, 32fps FP16, ~64fps INT8)
    modal run cloud/modal_train_plainnet.py

    # With perceptual loss
    modal run cloud/modal_train_plainnet.py --perceptual-weight 0.05

    # PlainDenoise variant
    modal run cloud/modal_train_plainnet.py --arch plain --nc 64 --nb 15

    # Quick test
    modal run cloud/modal_train_plainnet.py --max-iters 200 --val-freq 100

    # Resume previous run
    modal run cloud/modal_train_plainnet.py --resume
"""
import modal
import os
import time
...
def main(
    ...
    if checkpoint_dir:
        ...
        if arch == "unet":
            ...
            arch_tag += f"_nb{nb}"
        vol_ckpt_dir = f"{VOL_MOUNT}/checkpoints/plainnet_{arch_tag}"
